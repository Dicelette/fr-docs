...
De plus, il est possible d'utiliser la syntaxe `dé ; µ+x;µ*y` pour lancer un seul dé et appliquer différentes formules sur ce même dé, de façon à afficher le résultat de chaque formule. Le `µ` symbolise le résultat du dé.
...
Ces formules ne supportent pas la comparaison avec un résultat (`>=x` par exemple). Les commentaires entre chaque formule seront fusionnés en un seul commentaire.
