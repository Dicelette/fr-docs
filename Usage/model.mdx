...
:::usage
**`/calc [statistique] [signe] [expression] (*personnage) (commentaire)`**
:::

La commande demande de choisir, obligatoirement, la statistique à utiliser, le signe de l'opération à effectuer, et l'expression à appliquer.
Les signes valides sont : `+`, `-`, `*`, `/`, `^` (puissance), `%` (modulo), `/` (division), `≠` (différent de), `=` (égal à), `⩾` (supérieur ou égal à), `⩽` (inférieur ou égal à), `>` (supérieur à), `<` (inférieur à).
