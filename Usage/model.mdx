...
:::usage
**`/dbroll [statistique] (*personnage) (commentaire) (seuil_reussite) (modificateurs)`**
:::

La commande demande de spécifier :
- La statistique
- Le nom du personnage
- Les éventuels modificateurs (tels que l'avantage ou le désavantage), qui peuvent eux aussi des jets de dés incluant des formules, des noms de statistiques voire la valeur de la statistiques elle-même avec le mot clé `$`.
- Le remplacement du seuil de réussite (par exemple, pour un jet difficile), avec indication sur la comparaison (exemple : >= 10). Tout comme les modificateurs, il est possible de combiner des dés, des formules et des statistiques.
